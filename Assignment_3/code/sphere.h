...
public:
	...
	void paint() override {
		...
		glBegin(GL_QUADS);
		float phi = PI * 0.5f, nphi = phi - s_fPhiDelta;
		float theta, ntheta;
		if (s_bGouraud) {
	...
	static void SetSteps(uint32_t theta_steps, uint32_t phi_steps) {
		...
		s_fThetaDelta = (PI * 2) / s_nThetaSteps;
		s_fPhiDelta = PI / s_nPhiSteps;
	}

protected:
	inline Vec3f _GetPoint(float theta, float phi) {
		float cosp = cos(phi);
		return Vec3f(cosp * cos(theta), sin(phi), cosp * sin(theta));
	}
