...
void Object3D::insertIntoGrid(Grid* g, Matrix* m) {
	if (m_pBoundingBox) {
		const BoundingBox& aabb = m ? GetTransformedAABB(*m) : (*m_pBoundingBox);
		Indexs from, to;
		{
			const Vec3f& grid_min = g->getBoundingBox()->getMin();
			const Vec3f& voxel_size = g->GetVoxelSize();

			Vec3f from_index = aabb.getMin() - grid_min;
			Vec3f to_index = aabb.getMax() - grid_min;
		...
		for (int i = from[0]; i <= to[0]; i++) {
			for (int j = from[1]; j <= to[1]; j++) {
				for (int k = from[2]; k <= to[2]; k++) {
					g->InsertObjectIntoVoxel({ i, j, k }, this, m);
				}
			}
		}
	}
...
BoundingBox Object3D::GetTransformedAABB(const Matrix& m) { return _TransformBoundingBox(*m_pBoundingBox, m); }
